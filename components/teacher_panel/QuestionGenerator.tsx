...
const Select: React.FC<React.SelectHTMLAttributes<HTMLSelectElement>> = ({ children, className, ...props }) => (
  <select
    className={`p-2 bg-slate-700 rounded-md border border-slate-600 w-full disabled:opacity-50 text-white ${className}`}
    {...props}
  >
    {children}
  </select>
...
export const QuestionGenerator: React.FC = () => {
  ...
  const creditCost = useMemo(() => {
    if (userType !== 'authenticated') return 0;
    const baseCost = 1;
    const imageCost = (shouldGenerateImage && questionType === 'quiz') ? 1 : 0;
    return questionCount * (baseCost + imageCost);
  }, [questionCount, shouldGenerateImage, questionType, userType]);
  ...
  const handleGenerate = async () => {
    if (userType === 'authenticated' && aiCredits < creditCost) {
        setError(`Bu işlem için ${creditCost} kredi gerekiyor. Kalan krediniz: ${aiCredits}. Lütfen daha az soru seçin, görseli kapatın veya yarın tekrar deneyin.`);
        return;
    }
    ...
    try {
        ...
        if (userType === 'authenticated') {
            setAiCredits(prev => Math.max(0, prev - creditCost));
        }
    } catch (err: any) {
      ...
      <div className="space-y-4 bg-slate-800/50 p-4 rounded-xl border border-violet-500/30">
        ...
        <label className="flex items-center space-x-2 cursor-pointer text-slate-300">
          <input
            ...
            className="form-checkbox h-5 w-5 text-violet-600 bg-slate-700 border-slate-500 rounded focus:ring-violet-500"
            disabled={questionType !== 'quiz'}
          />
          <span>AI ile Görsel Oluşturulsun mu? (Yalnızca Çoktan Seçmeli / Soru Başına +1 Kredi)</span>
        </label>
        <Button 
        ...
        {userType === 'guest' && (
          <div className="text-center mt-3 p-3 bg-yellow-900/40 border border-yellow-500/50 rounded-lg">
            ...
            <p className="text-sm text-yellow-400/80 mt-1">Giriş yaparak soru üretebilir, kendi soru bankanızı oluşturabilir ve daha birçok özelliği kullanabilirsiniz!</p>
          </div>
        )}
