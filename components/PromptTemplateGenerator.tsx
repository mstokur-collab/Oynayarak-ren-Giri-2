...
export const PromptTemplateGenerator: React.FC = () => {
  ...
  return (
    <div className="p-4 sm:p-6 space-y-6">
      <div>
        <h3 className="text-xl font-bold text-violet-300 mb-3">AI Destekli Prompt Şablonu Üreticisi</h3>
      ...
      <div className="space-y-4 bg-slate-800/50 p-4 rounded-xl border border-violet-500/30">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <select value={grade} onChange={e => setGrade(parseInt(e.target.value))} className="p-2 bg-slate-700 rounded-md border border-slate-600 w-full">
            <option value="5">5. Sınıf</option>
            <option value="6">6. Sınıf</option>
            <option value="7">7. Sınıf</option>
            <option value="8">8. Sınıf</option>
          </select>
          <select value={ogrenmeAlani} onChange={e => setOgrenmeAlani(e.target.value)} className="p-2 bg-slate-700 rounded-md border border-slate-600 w-full col-span-1 sm:col-span-2">
            <option value="">Öğrenme Alanı Seçin</option>
            {ogrenmeAlanlari.map(oa => <option key={oa.name} value={oa.name}>{oa.name}</option>)}
          </select>
        </div>
        <select value={kazanımId} onChange={e => setKazanımId(e.target.value)} disabled={!ogrenmeAlani} className="p-2 bg-slate-700 rounded-md border border-slate-600 w-full disabled:opacity-50">
          <option value="">Kazanım Seçin</option>
          {kazanımlar.map(k => <option key={k.id} value={k.id}>{k.id} - {k.text}</option>)}
        </select>
        <Button onClick={handleGenerate} disabled={isLoading || !kazanımId} variant="violet" className="w-full !py-2 !text-base">
      ...
      {generatedTemplate && (
        <div className="space-y-4 animate-fadeIn">
            <h4 className="text-lg font-semibold text-violet-300">Üretilen Şablon:</h4>
            ...
            <Button onClick={handleCopy} variant="violet" className="w-full !py-2 !text-base">{copyButtonText}</Button>
        </div>
      )}
    </div>
  );
